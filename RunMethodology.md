...
```bash
# Install build-time dependencies (ultralytics for export, onnxruntime for depth quantization)
...
3. Builds TensorRT FP16 engine `triton/model_repository/yolo26n_seg/1/model.plan`
4. Quantizes Depth ONNX to `models/depth_anything_v2_vits.int8.onnx` (dynamic QUInt8)
5. Copies quantized Depth model to `triton/model_repository/depth_anything_v2_small/1/model.onnx`

**Optional: YOLO INT8 calibration with COCO data**

> Note: TensorRT 10.3 may reject the ORT QDQ graph for this YOLO export. Use only if you have confirmed compatibility.

```bash
ENABLE_YOLO_CALIBRATION=1 bash scripts/build_triton_engine.sh
```

Calibration reads images from `datasets/coco8/` (downloaded by `download_model.sh`). If calibration fails, the script exits with a clear error and the recommended fallback command.
...
- `triton/model_repository/yolo26n_seg/1/model.plan` — TensorRT FP16 engine (built from original ONNX by default)
- `models/depth_anything_v2_vits.int8.onnx` — dynamic QUInt8 quantized depth ONNX
- `triton/model_repository/depth_anything_v2_small/1/model.onnx` — copy of quantized depth model

**Optional: YOLO INT8 calibration**

```bash
ENABLE_YOLO_CALIBRATION=1 bash scripts/build_triton_engine.sh
```

Runs `scripts/yolo_calibrate.py` first to produce a QDQ INT8 ONNX, then passes `--int8` to trtexec. Requires `datasets/coco8/` (downloaded by `download_model.sh`). If calibration fails, the build exits with a clear error and recommended fallback.
